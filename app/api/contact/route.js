...
const contactFormScheme = z.object({
    fullname: z.string()
        ...
        .refine(val => val.trim().length >= 2, { message: "Full Name must be atleast 2 characters" })
    ...
    honeyset: z.string().optional().default(""),
})
...
export async function POST(request) {
    try {
        ...
        const validationResult = contactFormScheme.safeParse(formFields)
        ...
        if (uploadedFiles.length > 0) {
            try {
                ...
                console.log("File uploaded successfully: ", mediaFiles);


            } catch (error) {
                console.error("File Uploaded to WordPress failed: ", error);
