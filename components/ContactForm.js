...
const ContactForm = ({ subtitle, title, text }) => {
    ...
    const handleFileChange = e => {
        ...
        if (invalidFiles.length > 0) {
            setStatus({ type: "error", message: "Each file must be less than 10 MB" });
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                setStatus({ type: "success", message: "Thank You ! Your message has been send successfully. We will get back to you soon.." })
                ...
                if (data.details && Array.isArray(data.details)) {
                    ...
                    setStatus({ type: "error", message: data.error || "Something went wromg. Please Try Again" })
                }
            }
